...
export function initDirections(map) {
  ...
  wrapper.appendChild(panelGeo.onAdd(map));
  ...
  panelGeo.on('geolocate', e => {
    ...
    // 2) immediately clear any suggestions
    geocoderStart.clear();
    // 3) hide suggestions container if still visible
    const sugg = wrapper.querySelector('.mapboxgl-ctrl-geocoder--suggestions');
    if (sugg) {
