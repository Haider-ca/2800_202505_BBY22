 ...
 *   with PathPal’s functionality requirements.
 *
 * Sections:
 *   1. DOMContentLoaded & Initial Setup
 *   2. Toast Notification Setup
 *   3. Format Helpers
 *   4. Route Name Generation Utilities
 *   5. Profile Detection Helper
 *   6. Initial State Load from URL
 *   7. Save/Unsave Button Logic
 *   8. Clear Handler
 *   9. Polling for State Changes
 *  10. updateSaveButton Utility
 */
(function () {
  // 1. DOMContentLoaded & Initial Setup
  ...
  `;
    ...
    async function generateRouteName(routeGeoJSON) {
      ...
      return { display: `${shortenAddress(startFull)} → ${shortenAddress(endFull)}`, startFull, endFull };
    }
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      const geom = window.lastRouteGeoJSON?.geometry;
      if (!geom) {
        showToast('No route to save.'); return;
      }
      // Unsave existing route
      if (state.id) {
        try {
    ...
    setInterval(() => {
      ...
      // Seed URL-based route if newly loaded
